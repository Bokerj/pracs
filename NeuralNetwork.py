...
class NeuralNetwork():
    def __init__(self):
        ...
        self.wgh = {}  # Create dict to hold weights
        self.num_lyrs = 1  # Set initial number of layer to one (input layer)
        self.adjust = {}  # Create dict to hold adjustements

    def add_layer(self, shape):
        # Create weights with shape specified + biases
        self.wgh[self.num_lyrs] = np.vstack((2 * np.random.random(shape) - 1, 2 * np.random.random((1, shape[1])) - 1))
        # Initialize the adjustements for these weights to zero
        self.adjust[self.num_lyrs] = np.zeros(shape)
        self.num_lyrs += 1
    ...
    def predict(self, data):
        ...
        for layer in range(1, self.num_layers+1):
            data = np.dot(data, self.wgh[layer-1][:, :-1]) + self.wgh[layer-1][:, -1] # + self.biases[layer]
    ...
    def __forward_propagate(self, data):
        ...
        for layer in range(2, self.num_lyrs+1):
            data = np.dot(data.T, self.wgh[layer-1][:-1, :]) + self.wgh[layer-1][-1, :].T # + self.biases[layer]
    ...
    def __back_propagate(self, output, target):
        ...
        deltas[self.num_lyrs] = output[self.num_lyrs] - target

        # Delta of hidden Layers
        for layer in reversed(range(2, self.num_lyrs)):  # All layers except input/output
            a_val = output[layer]
            weights = self.wgh[layer][:-1, :]
        ...
        for layer in range(1, self.num_lyrs):
            self.adjust[layer] += np.dot(deltas[layer+1], output[layer].T).T

    def __gradient_descente(self, batch_size, learning_rate):
        # Calculate partial derivative and take a step in that direction
        for layer in range(1, self.num_lyrs):
            partial_d = (1/batch_size) * self.adjust[layer]
            self.wgh[layer][:-1, :] += learning_rate * -partial_d
            self.wgh[layer][-1, :] += learning_rate*1e-3 * -partial_d[-1, :]
    ...
    def train(self, inputs, targets, num_epochs, learning_rate=1, stop_accuracy=1e-5):
        ...
        for iteration in range(num_epochs):
            for i in range(len(inputs)):
                ...
                # Calculate the error
                loss = self.sum_squared_error(output[self.num_lyrs], y)
                error.append(loss)
...
if __name__ == "__main__":
    ...
    print('MeanError= ', np.mean(error[-4:]))
    print('It has to be trained = ', iteration,'times')
